#
# In the case of multi planes calibration, this dialog is used for
# changing the calibration parameters for one plane,
#
...
proc multi_calpar {} {
    ...
    # label $w.title -text "Calibration Parameters for plane: " -font $bold12 -width 20 -anchor center
    # pack $w.title -pady 3
    frame $w.title
    ...
    button $w.ok -text OK -command "change_plane 0; done_proc_cmd; destroy $w"
...
proc change_plane {iplane} {
	...
	# if {$iplane>0 && $iplane<=$cp(nplanes)} {}
	set i $iplane
	if {$i>0} {		;# show the values for the new plane
		set cp(plane_id) $i
