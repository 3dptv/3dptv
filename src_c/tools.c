...
double phimin;
	...
	time	= i/scan_par.f_cam;
	phimin	= -PI/scan_par.N;
	phi0	= phimin + 2 * PI * scan_par.f_pr * time /scan_par.N;
	phi1	= asin(1.0/scan_par.index* sin(phi0));
	scan_par.offset = scan_par.t * (sin(phi0)-(cos(phi0) * tan(phi1)));
}
