...
backtosequence:
		...
		if (counter2==1 || counter2==2) // CALC CENTRE
		{
			...
			if (yes==1)
			{
				...
				scanf("%lf",&scan_par.s); // bug fixed 17.06.2013 by Dominik, used to be: scanf("%f",&scan_par.s);
			}
	...
	printf("\nDo NEW nearest neighbour stuff? [1]=yes [2]=no");
	scanf("\n%d",&yes);
	if (yes==1)   // nearest neighbour stuff
	{
		...
		for (step2=seq_first; step2<seq_last; step2++)
		{
			...
			if ((step2-seq_first) % (scan_par.N_slice)== 0)
			{
				...
				/* sheet cycle */
				for (step=step2; step<(step2+scan_par.N_slice); step++)  // or -2?
				{
					...
					for (h=0; h<m[2]; h++)
					{ 
						// particle slice 1 in right half?
						if (mega[2][h].x[2] < z_cen_slice[step-step2])
						{
							...
							for (i=0; i<m[2]; i++)
							{
								// particle slice 2 in left half?
								if (mega[3][i].x[2] > z_cen_slice[step-step2+1])
								{
									X2 = mega[3][i].x[0];
									Y2 = mega[3][i].x[1];
									Z2 = mega[3][i].x[2];
									...
									if (distance < l_quader)
									{
										mega[2][h].decis[mega[2][h].inlist]=distance;
										mega[2][h].linkdecis[mega[2][h].inlist]=i;  //??
										mega[2][h].inlist++;	
									}
					...
					for (h=0;h<m[2];h++)
					{
						if(mega[2][h].inlist > 0 )
						{
							...
							if(mega[2][h].decis[0] < maxdist)
							{
								if(mega[2][h].x[2]<right) // particle of slice 1 is in slice 2
								{
								...
								{
									mega[3][mega[2][h].linkdecis[0]].finaldecis=mega[2][h].decis[0];
									mega[3][mega[2][h].linkdecis[0]].prev=h;
								}
							}
		...
		} // end NEW nearest neighbour stuff
