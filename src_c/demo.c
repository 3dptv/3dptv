...
int flow_demo_c (ClientData clientData, Tcl_Interp* interp, int argc, const char** argv)
{
  ...
  for (i_seq=seq_first; i_seq<=seq_last; i_seq+=scan_par.N_slice)
    {
//		for (j=-1;j<2;j++)
//		{
      compose_name_plus_nr (seq_name[nr], "", i_seq, name); //i_seq+j
      fp1 = fopen_r (name);	if (! fp1)	return TCL_OK;
      sprintf (buf, "display camera %d, image %d", nr+1, i_seq+j);  //i_seq+j
